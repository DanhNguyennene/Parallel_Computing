...
\caption{MPI Naive Performance (Placeholder)}
...
\caption{MPI Speedup Analysis (Placeholder)}
...
\caption{Hybrid MPI+OpenMP Performance (Placeholder)}
...
\caption{MPI Algorithm Comparison (Placeholder)}
...
\caption{MPI Efficiency Heatmap (Placeholder)}
...
\caption{MPI Strong Scaling Analysis (Placeholder)}
...
\begin{itemize}
    \item \textbf{GPU}: NVIDIA RTX 5070 Ti
...
\caption{GPU Shader Performance Comparison (Log Scale)}
\label{fig:gpu_perf}
\end{figure}
...
GPU Naive & 67.0ms & RTX 5070 Ti & 11.3× \\
GPU Chunked & 68.6ms & RTX 5070 Ti & 11.1× \\
GPU Strassen & 42.1ms & RTX 5070 Ti & 18.0× \\ \bottomrule
\end{tabular}
\end{table}
...
\section{Comprehensive Comparison: All Parallel Paradigms}

This section provides a detailed comparison between shared-memory (OpenMP), distributed-memory (MPI), hybrid (MPI+OpenMP), and GPU (OpenGL Compute Shader) parallelization approaches across all tested implementations.

\subsection{Algorithm Implementation Summary}

\begin{table}[H]
\centering
\caption{Algorithm Implementation Details Across Paradigms}
\label{tab:algo_summary}
\begin{tabular}{@{}llp{6cm}l@{}}
\toprule
\textbf{Paradigm} & \textbf{Algorithm} & \textbf{Key Characteristics} & \textbf{Parallelization} \\ \midrule
\multirow{2}{*}{OpenMP} & Naive & i-k-j loop ordering, SIMD vectorization & Thread-level (1-28 threads) \\
& Strassen & Recursive divide-and-conquer, threshold=128 & 7, 14, 21, 28 threads \\ \midrule
\multirow{2}{*}{MPI} & Naive & Row-wise decomposition, broadcast matrix B & Process-level (1-8 procs) \\
& Strassen & 7-process fixed, each computes one $M_i$ product & Exactly 7 processes \\ \midrule
Hybrid & Strassen & MPI for 7 products + OpenMP within each & 7 MPI × variable threads \\ \midrule
\multirow{3}{*}{GPU Shader} & Naive & Element-wise, global memory only & 1000s of GPU threads \\
& Chunked & 16×16 tiling with shared memory & Workgroup parallelism \\
& Strassen & Single-level, 7 kernel invocations & Host-orchestrated GPU \\ \bottomrule
\end{tabular}
\end{table}

\subsection{Performance Overview - All Implementations}

\subsubsection{Small Matrices (100-128×100-128)}

\begin{table}[H]
\centering
\caption{Best Performance for Small Matrices}
\begin{tabular}{@{}lccl@{}}
\toprule
\textbf{Implementation} & \textbf{Configuration} & \textbf{Time (ms)} & \textbf{Notes} \\ \midrule
OpenMP Naive & 2 threads & 1.72 & Minimal overhead \\
OpenMP Strassen & 7 threads & 3.13 & Algorithm overhead high \\
MPI Naive & 1 process & 0.092 & Serial performance \\
MPI Strassen & 7 processes & 148.6 & Severe overhead \\
GPU Naive & RTX 5070 Ti & 2.62 & Best GPU option \\
GPU Strassen & RTX 5070 Ti & 2.75 & Comparable to naive \\ \bottomrule
\end{tabular}
\label{tab:perf_small}
\end{table}

\textbf{Winner: OpenMP Naive (2 threads)} - Shared memory with minimal synchronization overhead provides best performance for small problem sizes.

\subsubsection{Medium Matrices (1000-1024×1000-1024)}

\begin{table}[H]
\centering
\caption{Best Performance for Medium Matrices (1024×1024)}
\begin{tabular}{@{}lccl@{}}
\toprule
\textbf{Implementation} & \textbf{Configuration} & \textbf{Time (ms)} & \textbf{Speedup vs Serial} \\ \midrule
Serial CPU (reference) & — & 758.5 & 1.00× \\
OpenMP Naive & 16 threads & 100.9 & 7.52× \\
OpenMP Strassen & 7 threads & 180.0 & 4.21× \\
MPI Naive & 2 processes & 83.4 & 9.09× \\
MPI Strassen & 7 processes & 1868.3 & 0.41× (slower!) \\
Hybrid Strassen & 7 proc + 2 threads & 436.0 & 1.74× \\
GPU Naive & RTX 5070 Ti & 67.0 & 11.32× \\
GPU Chunked & RTX 5070 Ti & 68.6 & 11.06× \\
\textbf{GPU Strassen} & \textbf{RTX 5070 Ti} & \textbf{42.1} & \textbf{18.01×} \\ \bottomrule
\end{tabular}
\label{tab:perf_medium}
\end{table}

\textbf{Winner: GPU Strassen} - Combines algorithmic advantage ($O(n^{2.807})$) with massive GPU parallelism. MPI Strassen performs poorly due to network overhead exceeding algorithmic benefits.

\subsubsection{Large Matrices (8192×8192)}

\begin{table}[H]
\centering
\caption{Best Performance for Large Matrices (8192×8192)}
\begin{tabular}{@{}lccl@{}}
\toprule
\textbf{Implementation} & \textbf{Configuration} & \textbf{Time (s)} & \textbf{Notes} \\ \midrule
GPU Strassen & RTX 5070 Ti & 14.73 & Best absolute performance \\
GPU Chunked & RTX 5070 Ti & 16.72 & Tiling optimization \\
GPU Naive & RTX 5070 Ti & 20.01 & Timeout issues \\ \bottomrule
\end{tabular}
\label{tab:perf_large}
\end{table}

\textit{Note: CPU implementations not tested at this size due to excessive runtime (estimated $>$1 hour for serial).}

\textbf{Winner: GPU Strassen} - GPU dominates at large scales with 1.36× speedup over naive, demonstrating combined benefit of Strassen algorithm and GPU acceleration.

\subsection{Scalability Analysis}

\begin{table}[H]
\centering
\caption{Parallel Efficiency Comparison (1000×1000 matrix)}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Implementation} & \textbf{Units} & \textbf{Time (ms)} & \textbf{Speedup} & \textbf{Efficiency (\%)} \\ \midrule
\multirow{4}{*}{OpenMP Naive} & 1 thread & 79.7 & 1.00 & 100.0 \\
& 2 threads & 35.5 & 2.24 & 112.2 \\
& 4 threads & 20.4 & 3.91 & 97.6 \\
& 8 threads & 20.4 & 3.91 & 48.9 \\ \midrule
...
GPU (3072 cores) & RTX 5070 Ti & 67.0 & 11.32 & 0.37 per core \\ \bottomrule
\end{tabular}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Best Choice:} GPU Strassen (if single GPU suffices) or Hybrid MPI+OpenMP (for multi-node clusters)
        \item \textbf{Reason:} Computation dominates communication, GPU memory bandwidth + Strassen algorithm combine optimally
        \item \textbf{Limitation:} GPU timeout at $N \geq 16384$ requires multi-pass or distributed GPU approach
    \end{itemize}
    
    \item \textbf{Distributed Systems:}
    \begin{itemize}
        \item \textbf{Fast Interconnect (InfiniBand):} Hybrid MPI+OpenMP recommended
        \item \textbf{Slower Network (Ethernet):} OpenMP on single node may be preferred depending on network performance
        \item \textbf{Critical:} MPI Strassen requires 7 processes - not flexible for arbitrary process counts
    \end{itemize}
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{Pros and Cons of Each Parallel Paradigm}
\label{tab:tradeoffs}
\begin{tabular}{@{}llp{5cm}p{5cm}@{}}
\toprule
\textbf{Paradigm} & \textbf{Strengths} & \textbf{Weaknesses} & \textbf{Best Use Case} \\ \midrule
OpenMP & Low overhead, excellent scaling up to 8 cores, easy programming & Limited to single node, memory bandwidth bottleneck & Medium matrices on workstations \\ \midrule
MPI & Scales to many nodes, unlimited total memory & High communication overhead, complex programming & Very large problems on HPC clusters \\ \midrule
Hybrid & Combines benefits of both, optimal resource utilization & Most complex to implement & Large matrices on multi-node systems \\ \midrule
GPU & Massive parallelism, high memory bandwidth & Kernel launch overhead, driver timeouts, limited memory & Medium-to-large matrices with GPU available \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Development Effort and Debugging Difficulty}
\begin{tabular}{@{}lcccl@{}}
\toprule
\textbf{Implementation} & \textbf{LOC} & \textbf{Complexity} & \textbf{Debug Difficulty} & \textbf{Key Challenges} \\ \midrule
OpenMP Naive & 150 & Low & Low & Thread synchronization \\
OpenMP Strassen & 300 & Medium & Medium & Recursive parallelism \\
MPI Naive & 250 & Medium & Medium & Data distribution \\
MPI Strassen & 400 & High & High & 7-process coordination \\
Hybrid & 450 & Very High & Very High & Two-level parallelism \\
GPU Naive & 50 & Low & Medium & Shader debugging \\
GPU Strassen & 200 & High & High & Multi-kernel orchestration \\ \bottomrule
\end{tabular}
